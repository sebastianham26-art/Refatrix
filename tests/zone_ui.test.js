...
async function openCheck(SHIP) {
  const ctx = mkDom('refatrix-inbound.html', {
    ...
    expose: 'window.__t={openShip:openShip,renderDetail:renderDetail,setStep:function(x){STEP=x;},lockPallet:lockPallet,zoneBigHtml:zoneBigHtml,zoneChip:zoneChip};',
  });
  ...
  ctx.w.__t.lockPallet(11);   // 개편(20260817m): 팔렛을 먼저 선택해야 스캔 입력이 열린다
  ctx.scan = (raw) => {
