...
export default async function productRoutes(app) {
  ...
  app.get('/api/products', { preHandler: [authGuard, requirePage('products')] }, async (req) => {
    ...
    const rows = (await query(
      `SELECT p.id, p.code, p.scode, p.app, p.ean, p.name, p.list_price, p.discount, p.iva_rate,
         ...
         LEFT JOIN (
           SELECT pa.product_id, MIN(v.rank) AS vio_rank
             FROM product_applications pa
             JOIN vio_models v
               ON v.model_key = pa.model_key
               OR v.model_key = pa.model_stem
            GROUP BY pa.product_id
         ) vio ON vio.product_id = p.id
        WHERE ${where}
        ORDER BY ${orderBy}
    ...
    // 각 행을 권한에 맞게 최소화. vio_rank는 숫자로 강제(없으면 null).
    return {
      items: rows.map((p) => minimizeProduct(perm, { ...p, vio_rank: p.vio_rank == null ? null : Number(p.vio_rank) })),
      limit, offset, total,
    };
  });
  ...
  app.post('/api/products/import/commit', { preHandler: [authGuard, requireDirector] }, async (req, reply) => {
    ...
    async function syncApp(c, productId, applications) {
      ...
      for (const a of (applications || [])) {
        await c.query(
          `INSERT INTO product_applications (product_id, app_text, maker, model, year_from, year_to, model_key, model_stem) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
          [productId, a.app_text, a.maker, a.model, a.year_from, a.year_to, a.model_key || null, a.model_stem || null]);
      }
    }
  ...
  // 이미 올린 제품들의 분해 데이터를 한 번에 채울 때 사용. model_key(VIO 매칭키)도 함께 재생성한다.
    ...
    for (const pr of prods) {
      ...
      await withTx(async (c) => {
        ...
        for (const a of apps) {
          await c.query(
            `INSERT INTO product_applications (product_id, app_text, maker, model, year_from, year_to, model_key, model_stem) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
            [pr.id, a.app_text, a.maker, a.model, a.year_from, a.year_to, a.model_key || null, a.model_stem || null]);
        }
      });
      n++;
    }
    // VIO 매칭 커버리지 진단: 매칭률과 미매칭 상위 차종을 함께 반환(시노님/정규화 튜닝용).
    let coverage = null;
    try {
      const tot = Number((await query(
        `SELECT COUNT(*)::int AS n FROM product_applications WHERE model_key IS NOT NULL AND model_key <> ''`)).rows[0].n);
      const matched = Number((await query(
        `SELECT COUNT(*)::int AS n FROM product_applications pa
          WHERE EXISTS (SELECT 1 FROM vio_models v WHERE v.model_key = pa.model_key OR v.model_key = pa.model_stem)`)).rows[0].n);
      const prodMatched = Number((await query(
        `SELECT COUNT(DISTINCT pa.product_id)::int AS n
           FROM product_applications pa
          WHERE EXISTS (SELECT 1 FROM vio_models v WHERE v.model_key = pa.model_key OR v.model_key = pa.model_stem)`)).rows[0].n);
      const unmatched = (await query(
        `SELECT pa.maker, pa.model, COUNT(*)::int AS n
           FROM product_applications pa
          WHERE pa.model_key IS NOT NULL AND pa.model_key <> ''
            AND NOT EXISTS (SELECT 1 FROM vio_models v WHERE v.model_key = pa.model_key OR v.model_key = pa.model_stem)
      ...
      coverage = {
        ...
        match_rate: tot ? Math.round((matched / tot) * 1000) / 10 : 0,
        products: n,
        products_with_rank: prodMatched,
        top_unmatched: unmatched.map((r) => ({ maker: r.maker, model: r.model, count: Number(r.n) })),
      };
    } catch (e) { coverage = { error: String(e.message || e) }; }
    await logEvent({ userId, action: 'update', target: 'product_resync', detail: { products: n } });
    return { ok: true, products: n, vio_coverage: coverage };
  });
}
