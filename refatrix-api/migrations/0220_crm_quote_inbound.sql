...
VALUES (
  ...
  '{"fields":[
      {"name":"rfc","type":"string","required":true,"es":"RFC del cliente — asi identificamos al cliente en el ERP. NO aceptamos customer_id / customerId / id (numericos): el cliente 5 del CRM no es el cliente 5 del ERP","ko":"고객 RFC (필수) — 고객을 찾는 열쇠. customer_id 같은 숫자는 받지 않는다(읽지도 않는다)"},
      {"name":"lineas | lines | partidas","type":"array","required":true,"es":"Lineas de la cotizacion. Aceptamos los tres nombres","ko":"견적 줄 배열 — 세 이름 다 받는다"},
      {"name":"lineas[].codigo | code | sku","type":"string","required":true,"es":"Codigo CTR o codigo SYD equivalente","ko":"CTR 코드 또는 SYD 코드 — 이름 세 가지 다 받는다"},
      {"name":"lineas[].cantidad | qty | piezas","type":"number","required":true,"es":"Cantidad solicitada (entero > 0)","ko":"수량 — 이름 세 가지 다 받는다"},
      {"name":"cotizacionCrm | folio | quoteNo","type":"string","required":false,"es":"Folio del portal (COT-...). ESTE MISMO folio queda como numero de cotizacion en el ERP, para que los dos sistemas hablen del mismo numero. Ademas evita duplicados: si reenvian el mismo folio devolvemos la misma cotizacion. Si no viene como campo, lo extraemos del texto de memo/comentario (menos fiable)","ko":"포털 견적번호 — **이 번호가 그대로 ERP 견적번호가 된다**(두 시스템이 같은 번호를 쓴다). 동시에 **중복 방지 키**. 전용 필드가 없으면 memo 문구에서 COT-… 를 주워 온다(차선책)"},
      {"name":"clienteCrm | customerCode","type":"string","required":false,"es":"Codigo del cliente en el CRM (alternativa al RFC)","ko":"CRM 고객코드 — RFC 대신 쓸 수 있다"},
      {"name":"fecha | quote_date | quoteDate","type":"string","required":false,"es":"Fecha de la solicitud (YYYY-MM-DD). Por omision, hoy","ko":"견적일자 — 비우면 오늘. quote_date 도 알아본다"},
      {"name":"comentario | memo | mensaje","type":"string","required":false,"es":"Comentario del cliente","ko":"고객 메모 — memo 도 알아본다"},
   ...
   "raw":"",
   "notes":"견적번호는 포털 번호(COT-…)를 **그대로** 쓴다 — 고객이 전화로 말한 번호로 바로 찾을 수 있게. 번호가 없거나 모양이 이상하면 우리 번호(Q-YYYY-####)로 되돌아가고 원문은 보존된다. 고객은 RFC(또는 CRM 고객코드)로만 찾는다 — 숫자 id 는 받지 않는다(남의 고객에 붙는 사고를 막기 위해). 같은 cotizacionCrm 을 다시 보내면 견적은 새로 생기지 않고 기존 번호를 돌려준다. 못 찾은 코드·판매중단 SKU 가 있어도 견적은 만들어지되 그 줄이 표시되고 **확정이 잠긴다**."}'::jsonb,
  19)
