...
export function buildMbrPrompt(snaps) {
  ...
  return [
    ...
    '',
    '보고서의 맨 첫 줄은 (어떤 섹션 제목보다도 앞에) 반드시 다음 형태의 한 문장 총평으로 시작하라:',
    '**이번 달엔 ~의 성과가 있었고, 다음 달에는 ~이 기대됩니다.**',
    '(~ 부분은 자료에서 가장 중요한 성과와 다음 달 기대 포인트로 채울 것. 굵게(**) 처리한 한 문장만, 다른 말 붙이지 말 것.)',
    '',
    '가장 중요한 원칙: 이 보고서는 단순 나열이 아니라 경영 인사이트 보고서다.',
    '모든 섹션을 "무엇을 했고(완료) → 무엇이 남았고(펜딩) → 다음에 무엇을 해야 하며(액션) → 실행하면 무엇이 개선되는가(기대효과)"의 흐름으로 서술하라.',
    '',
    '작성 규칙:',
    '- 반드시 아래 6개 섹션 제목(### 포함)을 그대로 사용할 것.',
    '- 자료에 없는 사실·숫자를 지어내지 말 것. 자료가 부족한 섹션은 "자료 없음"이라고 쓸 것.',
    '- 표는 쓰지 말고 짧은 문장과 불릿(-)만 사용할 것.',
    '- 숫자 나열은 최소화하고, 각 숫자가 말해주는 변화·원인·의미를 한 문장으로 해석해 붙일 것(예: 달성률 갭이 어느 주차에서 벌어졌는지).',
    '- 이슈 분류 기준(주차 비교로 추론할 것):',
    '  · 여러 주에 반복 등장해 마지막 회의까지 남아 있는 항목 = ⏳ 펜딩(미해결, 몇 주째 반복인지 표시)',
    '  · 이전 주에 있다가 이후 회의에서 사라진 항목 = ✅ 완료로 추정(추정임을 명시)',
    '  · 마지막 회의의 "다음주" 항목 = ▶ 다음 액션',
    '- 4번 섹션은 부서(영업/영업지원/제품마케팅/창고/경영총괄)마다 "#### 부서명" 소제목 아래 이 형식을 지킬 것:',
    '  **한줄 요약:** (그 부서의 이번 달을 한 문장으로)',
    '  - ✅ 완료: 이번 달 한 일',
    '  - ⏳ 펜딩: 해결 안 되고 남은 일',
    '  - ▶ 다음 액션: 다음 달 해야 할 일',
    '  - 📈 기대 개선: 다음 액션을 실행하면 무엇이 좋아지는가',
    '  이슈 기록이 전혀 없는 부서는 "**한줄 요약:** 기록된 활동 없음" 한 줄만 쓸 것.',
    '- 6번 섹션의 각 과제 뒤에는 "→ 실행 시 개선되는 것"을 화살표로 붙일 것.',
    '',
    '### 1. 월간 핵심 요약',
    '### 2. 매출·수금 실적 추이',
    '### 3. 고객 개발·견적 현황',
