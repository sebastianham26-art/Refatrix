...
export default async function integrationRoutes(app) {
  ...
  app.post('/api/integrations/:key/test', guard, async (req, reply) => {
    ...
    if (!payload && b.customer_id) {
      const c = (await query(
        `SELECT rfc, discount, credit_days FROM customers WHERE id=$1`, [Number(b.customer_id)])).rows[0];
      if (!c) return reply.code(404).send({ error: 'customer_not_found' });
      if (!String(c.rfc || '').trim()) return reply.code(400).send({ error: 'rfc_missing', note: '이 고객은 RFC 가 없어 시험 전송할 수 없습니다.' });
    ...
    if (!payload) {
      ...
      try { payload = sample ? JSON.parse(sample) : null; } catch (_) { payload = null; }
      if (!payload) return reply.code(400).send({ error: 'payload_required', note: '계약서의 요청 예시가 비어 있습니다. 고객을 선택하거나 예시를 채우세요.' });
    }
    ...
    return {
      ok,
      request: { method: r.method, url: r.url, env: ep.env, payload, auth: !!ep.auth_token },
