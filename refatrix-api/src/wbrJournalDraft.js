...
//   ⚠ 오분류가 잦으면 여기 문구만 고치면 된다. 화면에서 디렉터가 문장별로 다시 고를 수 있다.
const ORG_SCOPE = {
  sales: '고객을 직접 상대하는 활동 — 방문·상담·견적/가격 협상·수주·신규 거래처 개발·수금 독촉·영업사원 관리',
  support: '수주 이후의 사무·처리 업무 — 주문 등록, 출고 서류·인보이스·크레딧노트, 고객 등록/RFC, 클레임·반품 처리, CRM·ERP·시스템/전산 작업, 타팀 지원 요청 처리',
  pm: '팔 물건과 그 알림 — 신제품·품번 개발, 카탈로그·웹카탈로그, 가격표 작성, 경쟁사·시장 조사, 전시회, 광고·SNS·마케팅 지출',
  wh: '물건의 물리적 흐름 — 입고·출고, 재고·재고실사, 랙/로케이션·창고 이동, 포장·배송·운송, 수입 통관·컨테이너',
  mgmt: '회사 운영 — 인사·채용·급여·조직, 자금·재무·환율·은행, 본사/투자자 보고, 법무·계약·세무, 전사 전략 결정',
};

// 헷갈리기 쉬운 경계 — 실제 오분류를 줄이기 위한 판정 규칙.
const ORG_TIEBREAK = [
  '한 문장에 여러 부서가 섞여 있으면 **그 일을 실제로 처리한 쪽**을 고른다. 예: "영업이 요청한 인보이스를 발행" → 발행이 핵심이므로 support.',
  '고객과의 대화·협상·방문 = sales. 그 결과로 생긴 서류·등록·시스템 입력 = support.',
  '재고 "수치를 보고 판매 가능 여부를 판단" = sales / "실제로 세거나 옮기거나 넣고 뺐다" = wh.',
  '가격을 **고객과 협상** = sales / **가격표·단가표를 만들거나 고친다** = pm.',
  '수금 독촉·회수 협의 = sales / 입금 확인·전표·자금 계획 = mgmt.',
  '채용·급여·인사 면담은 대상이 영업사원이어도 mgmt.',
  'ERP·시스템·전산 개선 작업은 어느 팀 화면이든 support (단, 전사 도입 결정 자체는 mgmt).',
  '어느 쪽인지 정말 애매하면 mgmt 대신 **가장 구체적인 업무 부서**를 고른다. mgmt 는 회사 운영 사안 전용이다.',
];
...
export function buildDraftPrompt(entries, from, to) {
  ...
  return [
    ...
    cats,
    '',
    '### 카테고리가 헷갈릴 때의 판정 규칙 (중요 — 오분류가 가장 잦은 부분입니다)',
    ORG_TIEBREAK.map((s) => '- ' + s).join('\n'),
    '',
    '각 조직마다 두 칸을 채웁니다.',
