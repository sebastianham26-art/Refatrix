...
function boot(role='director', crossFlag=false){
  ...
  w.fetch = async (url, opt) => {
    ...
    if(/\/api\/customers\/\d+\/edit-basic/.test(u)) return J({item:{...CUST,id:9,team_id:2,team_name:'02_Merida',owner_id:6,owner_name:'Oscar'},in_scope:false,cross_team:true,pending:null});
    ...
    if(/\/api\/customers\/\d+$/.test(u) && m==='GET') return J({customer:CUST,invoices:[],summary:{year:2026,ytd_actual:0,year_target:null},reorder_summary:null});
