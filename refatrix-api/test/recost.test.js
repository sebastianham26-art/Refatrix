...
test('USD 환산 경로: USD 단가 × 입고일 환율 + USD 부대비용 환산 → computeRecost', () => {
  // prepareRecost가 하는 환산을 동일 규칙으로 재현: rate=17.5
  const rate = 17.5;
  const usdPrice = 10, usdOverhead = 200;             // 모두 USD 입력
  const priceMxn = r2(usdPrice * rate);                // 175
  const ovMxn = r2(usdOverhead * rate);                // 3500
  const res = computeRecost({
    productLines: { 1: [{ batch_id: 10, qty: 100, unit_price_mxn: priceMxn }] },
    batchOverhead: { 10: ovMxn }, batchTotalQty: { 10: 100 },
    productState: { 1: { stock_qty: 60, avg_cost: 0 } }, soldQty: { 1: 40 },
  });
  const p = res.perProduct[1];
  assert.equal(priceMxn, 175, 'USD 10 × 17.5 = 175 MXN');
  assert.equal(p.newAvg, 210, '새 원가 = 175 + (3500/100=35)');
  assert.equal(p.stockAddedMxn, 12600, '재고가산 60 × 210');
  assert.equal(p.retroCogsMxn, 8400, '소급 COGS 40 × 210');
});
