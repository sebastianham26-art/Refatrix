// =====================================================================
// 고객 등록 고도화 — CONSTANCIA 선점 + 디렉터 승인 + 기준품목 할인율 제안 (0185)
//
...
test('A4. RFC 형식은 느슨하게만 본다(현장 데이터가 지저분하므로 등록을 막지 않는다)', () => {
...
test('D3. 등록 API 가 CONSTANCIA 번호·스캔본·기준단가를 전부 요구한다', () => {
  for (const k of ['constancia_no_required', 'constancia_file_required', 'syd_ref_price_required']) {
    assert.ok(custRoutes.includes(k), `${k} 검증이 빠지면 증빙 없는 고객이 등록된다`);
  }
  // 스캔본 저장 실패 시 등록을 되돌리는지(번호만 있고 증빙 없는 고객 방지)
  assert.ok(custRoutes.includes('constancia_save_failed'));
...
test('D6. 엑셀 일괄 등록으로 신규 고객을 만드는 경로는 디렉터 전용', () => {
  assert.ok(/} else if \(!isDir\) \{\s*\n\s*blockedNew\+\+;/.test(custRoutes),
    '엑셀 업로드가 열려 있으면 CONSTANCIA 없이 고객을 대량 생성할 수 있다');
});

test('D7. 승인 시 CONSTANCIA 스캔본이 없으면 거부한다', () => {
  assert.ok(custRoutes.includes('constancia_missing'));
});
...
test('D9. 마이그레이션은 기존 고객을 approved 로 백필한다(회귀 방지)', () => {
  ...
  // RFC 는 기존 중복 데이터가 있으므로 유니크로 걸면 안 된다
  assert.ok(!/UNIQUE INDEX[^;]*rfc_norm/i.test(migration),
    'RFC 에 유니크를 걸면 기존 중복(지점 분리) 때문에 마이그레이션이 깨진다');
});
...
test('E1. 신규 등록 폼이 CONSTANCIA 번호·파일·기준단가를 막아 준다', () => {
  const save = custform.match(/async function save\(\)[\s\S]*?\n  \}/);
  assert.ok(save, 'save() 를 찾지 못했다');
  const s = save[0];
  assert.ok(s.includes('CONSTANCIA 번호를 입력해야'));
  assert.ok(s.includes('CONSTANCIA 스캔본(PDF)을 첨부하세요'));
  assert.ok(s.includes('기준품목의 고객 구매단가를 입력하세요'));
  assert.ok(s.includes('constancia_file'), '파일이 body 에 실려야 서버가 저장한다');
});

test('E2. 저장 직전 선점을 한 번 더 확인한다(입력 중 남이 먼저 등록한 경우)', () => {
  const save = custform.match(/async function save\(\)[\s\S]*?\n  \}/)[0];
  assert.ok(/var cc=await claimCheck\(true\)/.test(save));
  assert.ok(/cc\.blocked_constancia\|\|cc\.blocked_rfc/.test(save));
});
...
test('E8. 빌드 마커가 올라갔다(하드 리프레시 확인용)', () => {
  assert.ok(custform.includes('v20260826reg'));
  assert.ok(custHtml.includes('refatrix-custform.js?v=20260826reg'), 'custform 캐시버스터 동기화');
  assert.ok(custHtml.includes('reg-0826a'));
});
