...
test('D10b. 고객코드 발번은 삭제된 고객의 코드까지 센다', () => {
  ...
  // 0208 이후 채번은 src/customerCode.js 한 곳에 있다(CRM 수신의 P-#### 과 같은 규칙).
  const codeSrc = read(join(API, 'src/customerCode.js'));
  assert.ok(/SELECT code FROM customers`\)/.test(codeSrc),
    'computeNextCode 가 deleted_at IS NULL 로 걸러지면 반려 후 재등록이 막힌다');
  assert.ok(!/SELECT code FROM customers WHERE deleted_at IS NULL/.test(codeSrc));
  assert.ok(/from '\.\.\/customerCode\.js'/.test(custRoutes), '고객 등록이 공용 채번을 써야 한다');
});
