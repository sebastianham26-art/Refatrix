...
**산출물 zip**: `refatrix_inbound_line_v8.zip` (**오늘 존 지정 작업(0172)까지 누적** — zone v1 대체)
**빌드**: `refatrix-inbound.html` **20260817c → 20260817k**
...
4. **Ctrl+Shift+R** → 콘솔 `[refatrix-nav] v20260817z` · `[refatrix-inbound] build 20260817k` · `[refatrix-zones] build zone-0817a`.
...
curl -s ".../main/refatrix-inbound.html?nc=$(date +%s)" | grep -c "20260817k"                                # 2
...
## ③-4 입력칸 자동 처리 (build `20260817h` — 디렉터 보고: 리딩값이 커서에 남는다)

- **원인**: 입력칸 비우기가 Enter 키 처리에만 걸려 있어, 스캐너 전송 타이밍·IME 간섭으로 Enter 가 어긋나면 값이 남았다.
- **해결** (`wireScanInput` — 검수·적치목록·적치작업 3칸 공통):
  ① **처리보다 먼저 입력칸을 비운다** — 처리 중 무슨 일이 나도 잔여값 없음.
  ② **Enter 없이도 자동 처리** — 스캐너는 문자 간격 <80ms 로 쏟으므로, 전송이 140ms 멎으면 자동으로 읽고 비운다.
     접미사 설정·키 이벤트와 무관하게 항상 동작.
  ③ **손 타이핑 보호** — 입력 간격 >80ms(사람 속도)가 감지되면 자동 처리를 끄고 Enter/[스캔] 버튼을 기다린다.
- **중복 리딩**: 같은 코드 0.25초 내 재수신 시 이제 조용히 버리지 않고 **주의음 + "⚠ 중복 리딩 — 집계하지 않았습니다"** 토스트.


## ③-5 검수 일괄 스캔 모드 (build `20260817i→j` — 디렉터 재설계: "전체 스캔 후 버튼으로 처리")

**배경**: 한 팔렛 검수에서 수량이 자꾸 안 맞았다. 같은 SKU 박스들은 라벨이 전부 동일해 "이 박스를 이미 셌는지"를
시스템이 알 수 없고, 박스당 실시간 검증(팔렛 잠금·초과 차단)이 대기·오인을 만들었다.
**디렉터 결정**: *"박스는 자유롭게 전부 스캔하고, 마지막에 버튼을 눌러 팔렛을 계산한다."*

**새 검수 흐름** (`refatrix-inbound.html` 검수 탭 전면 재작성):
1. **자유 스캔** — 검증·차단 없음. 스캔마다 존 안내(크게) + 등록음 + "이 코드 누적 n회"만. 하단에 소입수별 집계표
   (`CE0796 ×16 — 20회`) 실시간 표시, 행별 [−] 오스캔 보정.
2. **[🧮 팔렛 처리 (n박스)]** → 팔렛 자동 판별(스캔 구성이 유일하게 맞는 팔렛). 모호하면 후보 칩 강조 → 탭 선택.
3. **정산표** — 라인별 예상/스캔/차이(✓/±n), 초과 스캔·미확인 코드 별도 표시, 수량 EA 대사.
4. **[✔ 검수 저장]** — 차이가 있으면 확인창(부족 검수 허용). 서버 전송은 기존 그대로 라인별 증분(`scanned_delta`),
   초과분은 전송되지 않음. [← 계속 스캔]으로 배치 유지한 채 복귀 가능(정산 중 스캔은 잠금).

**함께 유지**: 이중/연쇄 리딩 무시(경고음+토스트, 연쇄당 1회), 부속 바코드(숫자 8자리+) 무시, 미확인 코드는
차단 대신 별도 집계(오류음), 🕒 스캔 이력(최근 50건), 입력칸 자동 처리(Enter 불필요·즉시 비움).
**주의**: 스캐너가 자동(프레젠테이션) 모드면 같은 박스가 계속 재리딩되므로 **트리거 모드 권장** — 연쇄 차단이
막아주지만 1박스=1트리거가 가장 정확하다.


## ③-6 CARTON UNIT · QTY PER C/T 열 인식 (build `20260817k` + 백엔드 — 26B2C-4 수량 불일치의 원인)

**원인**: 파서가 카톤 수를 FROM/TO 번호 범위로만 계산했다. 패킹리스트에는 **CARTON UNIT(카톤 수)** 와
**QTY PER C/T(박스당 소입수)** 열이 따로 있고, `CARTON UNIT 2 × QTY PER C/T 12 = 24EA` 인 라인이
FROM/TO 기준으로는 1카톤 24EA 로 등재 → 소입수가 24로 계산돼 `CTR-…-12` 라벨과 안 맞았다(26B2C-4).

**파서 수정** (`parseWorkbook`):
- 카톤 수: **① CARTON UNIT 열(있으면 우선) → ② FROM/TO 범위 → ③ 0(낱개)**. 헤더 변형 대응
  (CTN UNIT·CARTON QTY·C/T UNIT / QTY PER CTN·QTY/C/T·PCS/CTN 등).
- TOTAL 열이 빈 양식이면 수량 = CARTON UNIT × QTY PER C/T 로 계산.
- CARTON UNIT×QTY PER C/T ≠ TOTAL 인 행은 미리보기에 **빨간 경고 목록**(파일 오류 조기 발견).
- 미리보기에 `카톤 기준: CARTON UNIT / FROM-TO` 표기.

**재분할 개선** (`applyRelines` — 백엔드 재배포 필요, 마이그레이션 없음):
- 검증 불변식을 **수량(qty)** 으로 변경 — 카톤 수는 교정 대상이므로 달라도 허용하고
  `inbound_pallets.cartons_expected` 를 파일 기준으로 갱신.
- **팔렛별 보호**: 검수/적치가 진행된 팔렛은 건너뛰고(스캔 기록 보존) 나머지만 교체. 응답에 `skipped` 목록.
- 프런트 🔀 버튼 노출 조건 완화: 미스캔 팔렛이 하나라도 있으면 표시(교정 재실행 가능).
- 독립 실행 도구도 v2 로 갱신(신형 파서 이식).

**26B2C 선적 처리 순서**: 배포(백엔드 포함) → 선적 열기 → [🔀 라인 재분할](또는 독립 도구) →
26B2C-4 가 2카톤 ×12 로 교정됨 → 전부 스캔 → [🧮 팔렛 처리] → 정산표 ✓.
...
## ⑤ 검증 (이 세션 — 누적 175/175 ✅ (검수는 일괄 모드 기준, relines 는 수량 불변·팔렛별 보호 기준))
...
- **`parser_cu.test.js` 11/11 (신규)** — CARTON UNIT×QTY PER C/T(26B2C-4 재현: FROM/TO=1이어도 2카톤·소입수 12)·TOTAL 빈 양식 계산·불일치 경고·구형 FROM/TO 회귀.
- **`scan_label.test.js` 45/45 (일괄 모드로 전면 재작성)** — 자유 누적·존 안내·미확인/부속/연쇄중복·집계표 −1·이력 / 팔렛 처리(자동 판별·소입수 라인 배정·정산표·저장 payload·저장음) / 모호 후보 칩 / 부족·초과 정산(확인창·상한 전송) / 계속 스캔 복귀 / 적치·재분할·입력 자동처리 회귀. — ⑫ 자동 처리 7건(Enter 유/무·타이핑 보호·중복 경고) 포함. — ⑪ 스캔 위생 15건(칩 잠금·이중리딩·부속 EAN·유예 취소/확정·후보 칩) 포함. — ⑩ 소리 7건(등록/주의/오류/저장음 구분·이벤트 시점) 포함. — ⑨ 라인 재분할 링크 노출 조건 3종 + POST 전송 포함. — 기존 43건 회귀(카운트 키를 라인 id 로 갱신) + **⑧ 멀티라인 12건**:
